...
import { Home, Compass, Settings, Users, LogOut } from 'lucide-react';
  ...
  return (
    <BrowserRouter>
      <div className="flex bg-slate-950 text-slate-100 font-sans h-screen w-screen overflow-hidden">
        ...
        {/* Sidebar - Only show if authenticated and not on landing/onboarding */}
        {isAuthenticated && (
          <nav className="w-16 md:w-64 glass-panel border-r border-slate-800/60 flex flex-col justify-between py-6 transition-all duration-300 z-50">
            <div className="px-4">
              ...
              <div className="flex flex-col gap-2">
                <NavigationLink to="/dashboard" icon={<Home className="w-5 h-5" />} label="Projects" active />
...
function NavigationLink({ to, icon, label, active = false }: { to: string, icon: React.ReactNode, label: string, active?: boolean }) {
  ...
  return (
    <RouteLink to={to} className={`flex items-center gap-3 p-3 rounded-xl transition-all border border-transparent hover:bg-slate-800/50 ${active ? activeClass : 'text-slate-400 hover:text-slate-100'}`}>
