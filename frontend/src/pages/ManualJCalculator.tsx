...
export default function ManualJCalculator() {
  ...
  const handleExportPdf = async () => {
    ...
    const drawStamp = () => {
      ...
      const stampSize = 60;
      const sx = pos.includes('right') ? pw - margin - stampSize : margin;
      const sy = pos.includes('bottom') ? ph - 40 - stampSize : 30;
    ...
    if (prefs.pdfIncludeLoadSummary !== false) {
      ...
      bHeaders.forEach((h, i) => doc.text(h, bCols[i], y, { align: i === 0 ? 'left' : 'right' }));
      y += 4;
      doc.setDrawColor(200); doc.setLineWidth(0.5);
