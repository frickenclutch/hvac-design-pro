...
function drawWatermark(doc: jsPDF): void {
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(
    'GENERATED BY HVAC DESIGNPRO \u2014 CLOUD ENGINEERING ENGINE',
    MARGIN + 2,
    PAGE_H - MARGIN - 2,
  );
}
...
function drawCoverPage(
  ...
  // PE Seal placeholder (dashed circle to left of title block)
  const sealX = tbX - 18;
  const sealY = tbY + tbH / 2;
  const sealR = 12.5;
  doc.setDrawColor(...GRAY_400);
  doc.setLineWidth(0.3);
  // Draw dashed circle approximation (16 segments)
  const segments = 16;
  for (let i = 0; i < segments; i++) {
    if (i % 2 === 0) {
      ...
      doc.line(
        sealX + sealR * Math.cos(a1),
        sealY + sealR * Math.sin(a1),
  ...
  doc.setFontSize(8);
  doc.setTextColor(...GRAY_400);
  doc.text('PE SEAL', sealX, sealY + 1, { align: 'center' });
}
...
export const generatePdfPlot = (
  ...
  // Page 4: Load Summary (conditional)
  let notesPageNum = 4;
  if (manualJ) {
    doc.addPage('a4', 'landscape');
  ...
  // Page 5 (or 4): Notes & Disclaimers
  doc.addPage('a4', 'landscape');
  drawNotesPage(doc, metadata, notesPageNum);
